...
The fix isn't always to remove the cache; sometimes you just need a freshness timestamp alongside the value and a consumer-side staleness gate. CaddieAI's `caddie.py` does exactly that for the workaround path: it reads the latch, computes age, and falls back if the age exceeds 1 s. Either approach beats silently returning the stale value.
...
## A repo-wide rename cannot reach the files the operator actually runs

> _Added 2026-07-21 from the CaddieAI `poc.py` -> `caddie.py` rename. Marc, on being shown the change: "I cannot understand why there are so many things that I think were done but weren't."_

I renamed the entry point across 80 tracked files, ran the suite green, and reported the documentation fixed "everywhere." An adversarial review then found the launcher Marc actually double-clicks - `run_caddieai_gui.bat` - still invoking `poc.py`. It was **gitignored**, machine-local to the sim-room PC. `git grep` cannot see it, the test suite cannot assert on it, and no commit can fix it. The first double-click after the merge would have died on `can't open file 'poc.py'`, with the product's primary launch surface simply gone.

The same review found the renamed entry point's OWN module docstring still teaching the retired interface, inside the same commit whose message claimed every such place had been corrected.

Three distinct failure modes, one root:

- **The tracked set is not the running set.** `git grep` answers "what does the repo say," never "what does this machine execute." Machine-local config, gitignored launchers, desktop shortcuts, scheduled tasks, and frozen build artifacts all invoke real code and are all invisible to every repo-wide tool. Ask *what actually starts this program* and follow each answer to a file, rather than trusting a search over version control.
- **A safety claim scoped to the wrong noun.** I kept the old `--gui` flag working so "nothing can break," and said so in the commit message. That protected the FLAG while I deleted the FILE. The mitigation was real, precisely targeted, and aimed at the wrong failure - which is more dangerous than no mitigation, because it produced confidence.
- **A sweep skips its own origin.** Mechanical replacement covers what the pattern matches; the file being renamed is where the human-written prose describing the old behavior lives, and prose is exactly what the pattern misses.

What actually worked was not more care. It was **making the change survive both states**: the launcher now dispatches `if exist caddie.py` -> new, else -> old, so it is correct before and after the merge, with no window in which the operator's shortcut is broken. When a change must land in a place version control cannot reach, the deliverable is not an edit that must be timed correctly - it is an artifact that is correct in every state.

This is the operational twin of "notes don't work - build gates": a gate you cannot run against the real launch surface is not a gate. The suite passing 186/186 said nothing whatsoever about whether the program would start.

> **Sign that this lesson applies:** I am renaming, moving, or deleting an entry point; or I have just written "everywhere" / "all references" / "nothing can break" in a summary; or my evidence that a change is complete is a `git grep` that came back empty. Ask which of the operator's real launch paths I have actually opened and read.
